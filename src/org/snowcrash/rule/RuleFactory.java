/*  
 * RuleFactory: Retrieves a rule for Critter adjudication. 
 * Copyright (C) 2010  Team Snow Crash
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the Artistic License/GNU GPL as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * Artistic License/GNU General Public License for more details.
 *
 * You should have received a copy of the Artistic license/GNU General 
 * Public License along with this program.  If not, see
 * <http://dev.perl.org/licenses/artistic.html> and 
 * <http://www.gnu.org/licenses/>.
 * 
 */

package org.snowcrash.rule;

/**
 * 
 * @author dearnest
 * Retrieves a rule for Critter adjudication.
 * 10/21/10	DE	Created.
 * 
 */

public class RuleFactory {
	
	private static RuleFactory instance = null;
	
	/**
	 * Returns an instance of the RuleFactory. 
	 * @return
	 */
	public static RuleFactory getInstance() {
		if (instance == null) {
			instance = new RuleFactory();
		}
		return instance;
	}
	
	/**
	 * Returns an instance of the rule based on the passed in Enum
	 * @param name
	 * @return
	 */
	public Rule getRule(Rules name) throws NoSuchRuleException {
		switch (name) {
			case DETECTING: 
				return new Detecting();
			case HUNTING:
				return new Hunting();
		}
		throw new NoSuchRuleException();
	}
	
}
